...
**Reading their resolution text changed the answer.** The plan was to settle "up or down"
against Pyth using the print at the window open and the print at the close. Their own
description rules that out:

> resolves to "Up" if the time-weighted average price (TWAP) of Dogecoin, generated by
> Chainlink, of the time range specified in the title is greater than or equal to the price
> at the beginning of that range … not according to any other sources or spot markets.

A Chainlink 60-second TWAP, not a Pyth print, and an average rather than a point. So the
Polymarket lane cannot be filled by teaching our parser one more title shape. It would
take adopting a second oracle for one venue, which destroys the only property that makes
this board mean anything: one referee, applied identically to everyone including us.

## The referee, and what it costs — 2026-08-27

Asking that question about Polymarket forced the same question about Kalshi, and the answer
had been sitting in their API the whole time. `KXBTCD` settles on CF Benchmarks' Real Time
Index, and their own note is exact: *"At the last minute before expiration, 60 RTI prices
are collected. The official and final value is the average of these prices."*

So: **no venue on this board settles on Pyth.** Kalshi averages sixty CF Benchmarks prints
over the final minute. Polymarket's short-horizon series takes a Chainlink 60-second TWAP.
This ledger takes one Pyth print at expiry.

That is a deliberate choice and it stays. Three venues asking different questions with
different strikes and different expiries can only be compared by one yardstick applied
identically — and applying our own settlement rule to ourselves as well as to them is the
only version that is not self-serving. But the choice has a cost, and the cost lands in
exactly one place: **near a strike, the verdict is decided by which oracle was asked.**

So near a strike we no longer publish a verdict. An observation is voided, and counted as
`inside-referee-band`, when its strike falls inside the interval a 60-second average could
plausibly have produced — the union of two things we already hold:

- **Pyth's own published confidence** around the settling print. The oracle stating what it
  does not claim to know is the least arguable band available.
- **the realised range of our samples over the final 60 seconds**, which is the window both
  venues average over. If the price visited both sides of the strike in that minute, an
  average could have come down on either side.

An exact tie is the limiting case and voids like any other, which is also the answer the
on-chain program gives an exact tie. This replaces the old inclusive-boundary convention:
resolving a boundary by convention was defensible when the referee was the only one in the
room, and is not defensible now that we know it is not the referee the venue used.

What this does *not* fix: away from the strike, the two oracles can still disagree, and a
row here is not the venue's own resolution. `/api/ledger` now says so in a `referee` field
that names each venue's settlement source, so nobody has to take this page's word for what
is being measured.
