# RESURRECTION.md — how anyone brings the machine back

This file exists so that RATCHET cannot die with its operator. If the site ever goes dark —
account lapsed, operator gone, hosting failed, whatever — **any stranger with this repo and a
...
PumpSwap — nobody, including the creator, can print, freeze, or pull it. The code: this repo,
...
has been tampered with cannot replay — the verifier tells you the exact entry where it breaks.

Keep copies. Snapshots are the machine's black box: whoever holds the latest one holds
everything needed.

## The resurrection, step by step (≈15 minutes, no permission needed)

1. **Get the code**: fork or download this repo.
2. **Verify the snapshot** you hold: `node restore.mjs snapshot.json --check` — it must print
...
3. **Create fresh state storage**: an Upstash Redis (free tier is fine) — copy its
...
It cannot recreate state lost BETWEEN the last snapshot and the outage — take snapshots often
if you care (they're one GET). It cannot stop a live operator from changing the rules — the
...
cannot force anyone to host the game — it only guarantees that anyone *may*, forever, with
proof.
