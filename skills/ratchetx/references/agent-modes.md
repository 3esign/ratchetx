# RatchetX agent modes: demo, ranked, verification

Read only when the user literally asks for a demo, ranked play, x402 entry,
MCP tools or proof links. Bankr/X play never uses anything here.
...
Bankr/X play and the owner-session test are described in `SKILL.md`; this
file covers the free demo, ranked play and verification.
