...
let publicPhase = 'startup';
const setPublicPhase = phase => { publicPhase = phase; };
function publicErrorKind(error) {
  const parts = [error?.code, error?.name, error?.message, error?.cause?.code, error?.cause?.message, String(error ?? '')]
    .filter(Boolean).join(' ');
  if (/RATCHET_G2_SEED|seed|configuration/i.test(parts)) return 'CONFIGURATION';
  if (/429|rate limit|too many requests/i.test(parts)) return 'RPC_RATE_LIMIT';
  if (/AbortError|timeout|fetch failed|ECONNRESET|ECONNREFUSED|ENOTFOUND|EAI_AGAIN|ETIMEDOUT|network/i.test(parts)) return 'NETWORK_OR_RPC';
...
async function runSeeded(command, flags, seed) {
  ...
  setPublicPhase('seed-fee-balance');
  const funding = await ensureDevnetFeeBalance(identity, web3, connection);
  setPublicPhase('seed-agent-create');
  const agent = createSeedAgent({ web3, connection, config: CONFIG, seed, onStatus: s => { setPublicPhase('seed-' + s.phase); if (process.env.RATCHET_G2_DEBUG) console.error('..', s.phase); } });
  if (command === 'preflight') { setPublicPhase('seed-preflight-load'); const view = await agent.game.load(); const credits = view.ledger?.credits ?? 0n, claimable = !view.ledger || (view.ledger.legacyCredits === 0n && view.ledger.legacyXp === 0n);
...
export async function runCli(argv, options = {}) {
  setPublicPhase('arguments');
  const rootDir = options.rootDir ?? stateRoot();
  const { command, flags } = argumentsFor(argv);
  if (command === 'diagnose') { setPublicPhase('diagnose'); return diagnoseResult(); }
  setPublicPhase('node-version');
  ...
  if (process.env.RATCHET_G2_SEED && command !== 'help') { setPublicPhase('seed-mode'); return runSeeded(command, flags, process.env.RATCHET_G2_SEED); }
  if (command === 'help') { setPublicPhase('help'); return { ok: true, code: 'HELP', reply: 'RatchetX G2 devnet: init --player PUBLIC_KEY; preflight; play --say USER_WORDS --command-id SOURCE_POST_ID; status; reconcile --command-id SOURCE_POST_ID; reveal --command-id SOURCE_POST_ID; finish --command-id SOURCE_POST_ID; recover-lock. Keep one persistent private state directory. Test credits only. Pyth prices · https://ratchetx.xyz/play' }; }
  if (command === 'init' || command === 'setup') { setPublicPhase('persistent-init'); return initialize(flags['--player'], rootDir); }
  // A public X command on a fresh host must not die on SETUP_REQUIRED: set up self-play and go on.
  if (!fs.existsSync(path.join(rootDir, PUBLIC_IDENTITY))) { setPublicPhase('persistent-auto-init'); initialize(undefined, rootDir); }
  setPublicPhase('persistent-identity');
  const identity = readPublicIdentity(rootDir);
  if (command === 'recover-lock') { setPublicPhase('recover-lock'); return recoverLock(identity, rootDir); }
  setPublicPhase('persistent-runtime-load');
  const web3 = loadWeb3(), connection = connectionFor(web3);
  if (command === 'preflight') { setPublicPhase('persistent-preflight'); return preflight(identity, web3, connection); }
  setPublicPhase('persistent-agent-load');
  ...
  try {
    if (command === 'status') { setPublicPhase('persistent-status'); return await runner.status(); }
    if (command === 'finish') { setPublicPhase('persistent-finish'); return await finishCommand(runner, flags['--command-id']); }
    if (command === 'play') {
      if (isSelfPlay(identity)) { setPublicPhase('persistent-self-credits'); const ready = await ensureSelfCredits(identity, web3, connection, keypair); if (ready.blocked) return ready.blocked; }
      else { setPublicPhase('persistent-fee-balance'); await ensureDevnetFeeBalance(identity, web3, connection); }
      setPublicPhase('persistent-play');
      return await runner.runCommand({ commandId: flags['--command-id'], text: flags['--say'] });
    }
    setPublicPhase('persistent-' + command);
