...
declare_id!("CnKAJQAQvJQ7Ht3rZRt4ZaFuZSFL4G6sDZShbmJUdTCx");
...
/// Devnet flavour only (`--features devnet`): the mint is a PDA of this program
/// (`[b"devnet-rcx"]`, bump 255) that anyone can create and mint from, so a
/// devnet pilot needs no token launch and no key. Never built into mainnet.
#[cfg(feature = "devnet")]
pub const RCX_MINT: Pubkey = pubkey!("9Becn57cNDJmRMrEBwtGbgfzY8eUF7VNjsgrQbfLpBRm");
#[cfg(feature = "devnet")]
pub const DEVNET_MINT_SEED: &[u8] = b"devnet-rcx";
...
pub mod ratchet_core {
    ...
    // ------------------------------------------------------- devnet only
    /// Create the devnet test mint (6 decimals, authority = itself). Idempotent
    /// by construction: a second call fails on `init`.
    #[cfg(feature = "devnet")]
    pub fn devnet_init_mint(_ctx: Context<DevnetInitMint>) -> Result<()> {
        Ok(())
    }

    /// Mint devnet test RCX to the caller's associated token account. Anyone,
    /// any amount up to 1,000,000 tokens per call.
    #[cfg(feature = "devnet")]
    pub fn devnet_faucet(ctx: Context<DevnetFaucet>, amount: u64) -> Result<()> {
        require!(amount > 0 && amount <= 1_000_000_000_000, CoreError::InvalidAmount);
        let bump = ctx.bumps.mint;
        let seeds: &[&[u8]] = &[DEVNET_MINT_SEED, &[bump]];
...
#[cfg(feature = "devnet")]
#[derive(Accounts)]
pub struct DevnetInitMint<'info> {
    #[account(
        init,
        payer = payer,
        seeds = [DEVNET_MINT_SEED],
        bump,
        mint::decimals = 6,
        mint::authority = mint,
...
pub struct DevnetFaucet<'info> {
    #[account(mut, seeds = [DEVNET_MINT_SEED], bump, address = RCX_MINT @ CoreError::WrongMint)]
    pub mint: InterfaceAccount<'info, Mint>,
    #[account(
        init_if_needed,
        payer = recipient,
