...
export const PROGRAM_ID = new PublicKey('CnKAJQAQvJQ7Ht3rZRt4ZaFuZSFL4G6sDZShbmJUdTCx');
export const RCX_MINT = new PublicKey('9Becn57cNDJmRMrEBwtGbgfzY8eUF7VNjsgrQbfLpBRm');
...
// ---- devnet faucet flavour (this build only) ----
export const DEVNET_MINT_PDA = PublicKey.findProgramAddressSync([Buffer.from('devnet-rcx')], PROGRAM_ID)[0];
export function devnetInitMintIx({ payer }) {
  const keys = [meta(DEVNET_MINT_PDA, false, true), meta(payer, true, true), meta(TOKEN_PROGRAM), meta(SystemProgram.programId)];
  return ix('devnet_init_mint', keys);
}
export function devnetFaucetIx({ recipient, amount }) {
  const keys = [meta(DEVNET_MINT_PDA, false, true), meta(ata(recipient), false, true), meta(recipient, true, true),
    meta(TOKEN_PROGRAM), meta(ATA_PROGRAM), meta(SystemProgram.programId)];
  return ix('devnet_faucet', keys, u64(amount));
}
