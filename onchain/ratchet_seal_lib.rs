...
const SETTLE_WINDOW_SECS: i64 = 60;
...
pub mod ratchet_seal {
    ...
    pub fn settle(ctx: Context<Settle>) -> Result<()> {
        ...
        require!(
            msg.publish_time >= shot.expiry_ts
                && msg.publish_time <= shot.expiry_ts + SETTLE_WINDOW_SECS,
        ...
        emit!(Settled { shot: shot.key(), exit_e6, publish_time: msg.publish_time, cranker: ctx.accounts.cranker.key() });
        Ok(())
    }
...
impl Shot { pub const SIZE: usize = 32 + 8 + 32 + 32 + 8*6 + 4; }
...
#[event] pub struct Settled  { pub shot: Pubkey, pub exit_e6: i64, pub publish_time: i64, pub cranker: Pubkey }
