...
const BACKUP = path.resolve(process.env.RATCHET_PRIVATE_BACKUP ||
  path.join(ROOT, 'backups', 'pre003-20260830-P7LEkP'));
...
const missingPrerequisites = [
  ...['public-schema.dump', 'ratchet-kv-data.dump']
    .filter(name => !fs.existsSync(path.join(BACKUP, name)))
    .map(name => 'backup:' + name),
  ...['initdb', 'pg_ctl', 'pg_restore']
    .filter(name => !fs.existsSync(exe(name)))
    .map(name => 'utility:' + name),
];
...
test('verified 2026-08-30 raw backup restores and satisfies the final analyzer', {
  ...
  assert.equal(missingPrerequisites.length, 0,
    'PRIVATE_RESTORE_PREREQUISITES_MISSING: ' + missingPrerequisites.join(', ')
    + '; set RATCHET_PRIVATE_BACKUP and RATCHET_PG_BIN to the verified private inputs');
