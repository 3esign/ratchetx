...
async function prepareBundle(shotId, digest) {
  ...
  if (receipt.result === 'INSUFFICIENT_EVIDENCE')
    return { errorStatus:receipt.code === 'RETAINED_EVIDENCE_EXPIRED' ? 410 : 503,
      error:receipt.code === 'RETAINED_EVIDENCE_EXPIRED'
        ? 'the keyless oracle observation is outside Ratchet retention'
        : 'keyless verification evidence is temporarily unavailable',
      detail:receipt.reason };
  if (receipt.result !== 'MATCH')
    return { errorStatus:422, error:'canonical record diverges from keyless audit verification', detail:receipt.reason };
  const run = await saveAgentRun({ shotId, receipt, chain:evidence.chain });

  const s = evidence.shot;
  const bundle = {
    bundleVersion:'ratchetx-proof-bundle-v2',
    ...
    verifier:{ source:`${publicOrigin()}/openapi.json`,
      mode:'keyless retained-evidence audit', independentPythReplay:false },
  };
