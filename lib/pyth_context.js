...
function parsePathRequest(input = {}, now = Date.now()) {
  ...
  const to = input.to == null ? now : Number(input.to);
  ...
  if (cursor) {
    try {
      ...
      if (!decoded || decoded.v !== 1 || !decoded.point
          || !Number.isFinite(Number(decoded.point.observedAt)))
        throw new Error('missing observedAt');
      const bound = decoded.request || {};
      if (bound.feed !== feed || bound.from !== from || bound.to !== to
          || bound.source !== source) throw new Error('request mismatch');
...
function pathResponse(request, points) {
  ...
  return {
    ...
    truncated,
    nextCursor:truncated && kept.length
    ...
    points:kept,
    pagination:'When nextCursor is present, repeat the same request with cursor=nextCursor. The composite cursor preserves distinct transitions captured in the same millisecond.',
