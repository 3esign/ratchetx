...
async function handle(req,res,ctx){
  ...
  try{
    ...
    if(!authorization.dispatch)return res.json({ok:true,idempotent:true,request:authorization.request,
    ...
    if(!result.body?.ok && result.status>=400 && result.status<500){
      ...
      try{await bridge.service.finish(permit,{state:'rejected',code:'SHOT_REFUSED'});}catch{}
    }
    let request;
    ...
    if(request?.state==='rejected')return res.status(409).json({ok:false,code:'SHOT_REFUSED',request});
