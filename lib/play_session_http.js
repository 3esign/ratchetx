...
const OWNER_OPS = new Set(['grant','revoke','owner-status','owner-discover','recover']);
// Player-lock contention: three tries, 2.4 s of waiting inside each plus this gap.
const BUSY_RETRIES = 2, BUSY_GAP_MS = 800;
...
// A session credential may only be issued by a store that survives a cold start
// and can refuse a stale write. That is the requirement; 'supabase' was only ever
// the name of the one store that met it.
//
// This is ONE predicate on purpose. The same question was being answered in two
// places -- the GET that advertises whether session creation is available, and
// the POST that enforces it -- and when the store moved to the Redis protocol on
// 2026-09-03 both said no. The play page reported that session creation was
// unavailable, the POST answered DURABLE_SESSION_STORE_REQUIRED, and every Bankr
// play stopped, for a backend that meets the requirement fully: casPlaySession
// there is a Lua compare-and-swap on the record's revision, atomic in one round
// trip, which is the same guarantee Supabase gave with a conditional PATCH.
//
// What must still be refused is the in-memory Map: a credential that evaporates
// on the next cold start is a promise the store cannot keep.
const sessionStoreReady = () => !!kv.durable && typeof kv.casPlaySession === 'function';
...
const SHOT_REFUSALS = Object.freeze({
  ...
  MIGRATION_FREEZE:'The machine is not selling new shots while the migration snapshot is taken; open shots settle normally.',
...
function refusalOf(request){
  ...
  // Owner recovery is a separately known terminal cause, not a game refusal.
  // Its retained result already says RECOVERED_NO_DISPATCH; do not relabel it.
  if(request.result?.code==='RECOVERED_NO_DISPATCH')return {};
...
const CODES = new Set(['G2_UPGRADE_REQUIRED', 'INVALID_WALLET','INVALID_GRANT','INVALID_WINDOW','INVALID_LIMITS',
...
async function handle(req,res,ctx){
  ...
  if(req.method==='GET')return res.json({ok:true,v:RELEASE,enabled:sessionStoreReady(),
    ...
    ownerDiscovery:{op:'owner-discover',action:'owner_discover',discovery:'latest-retained-session-v1',readOnly:true,
      requiresSessionId:false,requiresBearer:false,requiresOwnerSignature:true,
      signedFields:['domain','network','version','action','wallet','issuedAt','nonce'],
      nonce:'32 lowercase hex characters; bind the response to this owner lookup',
      effect:'Read only the latest retained session for the signing wallet, including revoked, expired or pending state. No player reads, settlement or new grants.',
      replay:'A repeated fresh signature is allowed and rereads current state; the nonce is not consumed.',
      missing:'A successful strict read with no retained row returns session:null; storage failure is not absence.'},
    limitBounds:{maxAttempts:100,maxStakeCredits:10000000,maxGrossCredits:100000000,maxDurationMs:86400000},
    agentContract:{method:'POST',contentType:'application/json',transport:'server-side HTTPS only; refuse redirects; no credentials in query/body/logs',
      ...
      shot:{body:{op:'shot',intent:{requestId:'32 lowercase hex characters; persist and reuse for the SAME intent only',
        target:'exact active board target id',side:'YES or NO',p:'number 0.01..0.99 in steps of 0.01',stake:'integer play credits 100..10000000, also bounded by your signed grant'}},
  ...
  throw Object.assign(new Error('RatchetX has upgraded to G2! Reply "@bankrbot update ratchetx" to install the new skill.'), {code: 'G2_UPGRADE_REQUIRED'});
  if(!sessionStoreReady())
    return res.status(503).json({ok:false,code:'DURABLE_SESSION_STORE_REQUIRED'});
  ...
  try{
    if(owner){
      ...
      if(b.op==='owner-discover'){
        if(Object.hasOwn(req.headers||{},'authorization'))fail('INVALID_CAPABILITY');
        return res.json({ok:true,readOnly:true,discovery:'latest-retained-session-v1',
          ...await bridge.service.ownerDiscover(b.payload,b.signature)});
      }
      if(b.op==='grant'){
    ...
    // A held player lock is a moment, not a verdict: the site open in a tab
    // polls the same wallet and settles under the lock, and the canonical
    // game waits only 2.4 s for it. On 2026-09-02 a public X reply died on
    // PLAYER_BUSY that way, and the rejected receipt burned the post. So a
    // busy lock is retried here before a reserved attempt is terminalized;
    // nothing was written by a refused lock, so the retry cannot double-play.
    let result;
    for(let attempt=0;;attempt++){
      bridge.markVerified(inner,permit,intent);
      result=await invoke(ctx.game,inner);
      if(result.body?.code!=='PLAYER_BUSY'||attempt>=BUSY_RETRIES)break;
    ...
    console.error('play_session_http error:', e);
    try { await kv.setnxJSON('dbg:error:' + Date.now(), { msg: String(e.message || e), stack: String(e.stack) }, 86400); } catch(err){}
    ...
    const status=code==='SESSION_UNAVAILABLE'?503:code==='AGENT_ADMISSION_REQUIRED'?403:
      ...
      /CONFLICT|CONTENTION|UNRESOLVED|WRITE_LEASE|UPGRADE_REQUIRED/.test(code)?409:401;
    return res.status(status).json({ok:false,code,error_message:String(e.message||e),error_stack:String(e.stack),
