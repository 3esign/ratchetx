...
function decideBurn(tx, { wallet, mint, minAmount = 1, nowMs = Date.now(), maxAgeSec = 86400,
  ...
  let decimals = 6;
  const firstBalance = [...pre, ...post].find(b => b.mint === mint);
  if (firstBalance && firstBalance.uiTokenAmount) {
    decimals = firstBalance.uiTokenAmount.decimals;
  }
  ...
  const allowedOwners = strictSets
    ? strictSets.flatMap(x => x.list.map(y => y && (y.owner || y.w)).filter(Boolean))
    : podium;
  ...
  let outflow = Math.max(0, -walletDelta);
  if (outflow === 0) {
    const signers = new Set(
      (tx.transaction && tx.transaction.message && tx.transaction.message.accountKeys || [])
        .filter(k => k.signer)
        .map(k => k.pubkey)
    );
    if (signers.has(wallet)) {
      let parsedOutflow = 0;
      ...
      for (const ix of [...outer, ...inner]) {
        ...
        if (!info || (parsed.type !== 'transfer' && parsed.type !== 'transferChecked')) continue;
        if (info.authority === wallet) {
          if (info.source === info.destination) continue;
          const amt = parsed.type === 'transferChecked'
            ? (info.tokenAmount && Number(info.tokenAmount.uiAmount) || 0)
            : (Number(info.amount) / (10 ** decimals) || 0);
          parsedOutflow += amt;
        }
      }
      outflow = Math.floor(parsedOutflow);
    }
  }
  ...
  if (podiumSet.has(wallet)) {
    ...
    for (const ix of [...outer, ...inner]) {
      const parsed = ix && ix.parsed, info = parsed && parsed.info;
      if (!info || (parsed.type !== 'transferChecked' && parsed.type !== 'transfer') || info.authority !== wallet
          || info.source !== info.destination) continue;
  ...
  if (destroyed + champPaid + selfRouted + 1e-6 < outflow) return { ok: false, reason: 'part of that transaction neither burned nor reached the incinerator/champions — send the burn to ' + INCINERATOR };
  ...
  if (strictSets) {
    const txMs = bt * 1000;
    const gross = Math.round(destroyed / (1 - podiumPct));
    ...
    for (const snap of strictSets) {
      ...
      for (const seat of snap.list) {
        ...
        if (amt > 0) expected.set(seat.owner || seat.w, amt);
      }
      
      // A retained share exists ONLY when this wallet holds a seat in THIS
      // snapshot — reload_build skips the reloader's own seat, so the seat's
      // tokens stay put and the reconstruction accounts for them here. For a
      // wallet with no seat, `gross - destroyed - champPaid` is nothing but
      // the ±1 rounding error of reconstructing gross from destroyed — and
      // attributing that error used to fabricate a ghost seat for the
      // reloader, so `expected.size !== observed.size` refused every
      // non-podium reload whose split carried rounding dust. The three
      // wallets testing in production all held seats, which is why this
      // never fired for them. (h69)
      const walletSeatAmt = expected.get(wallet) || 0;
      const virtualSelfRouted = selfRouted > 0 ? selfRouted
        : (walletSeatAmt > 0 ? Math.max(0, gross - destroyed - champPaid) : 0);
      const observed = new Map(champLegs.map(x => [x.w, Math.floor(x.amt)]));
      if (virtualSelfRouted > 0) observed.set(wallet, Math.floor(virtualSelfRouted));
      ...
      if (!same || Math.abs(Math.floor(destroyed) - Math.floor(gross - routed)) > 5) continue;
      
      selfRouted = virtualSelfRouted;
      podiumVersion = snap.v || snap.t || 'published';
      break;
    }
    if (podiumVersion == null) {
      // PURE BURNS ARE ALWAYS HONORED (h69). The oldest promise this game
      // makes — send RCX to the incinerator from any wallet, paste the
      // signature, get credits — predates the Champion's Cut. A transaction
      // with NO champion legs whose outflow verifiably left circulation gives
      // the machine 100% instead of the published 70/30, matches no podium
      // snapshot by construction, and harms nobody. Refusing it after the
      // tokens were already destroyed was the one shape of this function that
      // could keep a player's burn and pay nothing. Every transfer to a
      // wallet outside the podium was already refused above, so nothing here
      // widens what a reload may pay — it only stops a strictly-more-generous
      // burn from being punished for its generosity.
      if (champLegs.length === 0 && destroyed + 1e-6 >= outflow) {
        podiumVersion = 'pure-burn';
        selfRouted = 0;      // nothing was routed; a decorative self-transfer stays decoration
      } else {
