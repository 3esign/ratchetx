...
//  reveal after, exit price pinned to the first oracle publish at or after
//  expiry — so the row cannot be written after the fact and cannot be argued
//  with afterwards.
//
...
async function rows({ after = 0, limit = 200 } = {}) {
  ...
  for (const e of scan) {
    ...
    const recomputed = (ev.side && ev.salt) ? sha(`${ev.side}|${ev.salt}`) : null;
    ...
    out.push({
      ...
      commitVerified: (commit && recomputed) ? (recomputed === commit) : null,
...
const COLUMNS = ['schema', 'i', 'id', 'who', 'agent', 'feed', 'stake', 'entry',
  ...
  'commit', 'salt', 'sealed', 'commitVerified', 'reason'];
