...
const { CHUNK, logCount, readEntries, verifyStoredChain } = require('./log.js');
...
async function findSettledShot(id) {
  ...
  const list = await readEntries(issued);
  ...
  const head = await getJSONStrict('g:log:head');
  const verification = head || issued === 0
    ? verifyStoredChain(list, head, issued)
    : { ok:false, intact:false, count:list.length, brokenAt:issued,
      missing:[], reason:'event log head is missing' };
