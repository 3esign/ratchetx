...
async function append(ev) {
  ...
  // Permanent immutable retention. Unlike a shared chunk blob, no concurrent
  // writer can overwrite this entry because exactly one appender owns index i.
  await setJSON(`g:log:e:${i}`, { ...entry, h });

  const newHead = { i, h };
  // never roll the published head backwards for a slow appender
  // A transient read failure must never look like an empty head here: a slow
  // older appender could otherwise publish its lower index over a newer one.
  const curHead = await getJSONStrict('g:log:head');
