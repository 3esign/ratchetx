...
const { verifyLegacy } = require('./legacy_chain.js');
...
async function append(ev, onceKey = null) {
  ...
  try {
    ...
    // c:1 marks an entry hashed over CANONICAL bytes. It is written into the
    // body, not inferred from an index, so an entry always carries the rule it
    // was made under and no constant can drift away from the data.
    const entry = { i, t:Date.now(), ev, c:1 };
    const h = sha(prev + canon(entry));
...
const DISCLOSED_GAPS = Object.freeze({
  345: Object.freeze({
    ...
    // Recovered 2026-08-27 and proven by the chain itself: stored h346 only
    // reproduces with this content present. Deliberately NOT used to verify the
    // 345/346 link, and deliberately not written back — from the STORED data
    // alone that link is unprovable, and that is what this verifier reports.
    // Reproduce with tools/recover-345.mjs. Story in docs/CHAIN_GAP.md.
    recoveredContent: '{"i":345,"t":1787277604359,"ev":{"k":"wsettle","id":"w496454","outcome":true,"hit":false,"exitPx":2358.55559871}}',
    recoveredHash:    'b2637f799c7cc65b4db3c308e2a8b785b505e81d5f63b0ce79dd5ec3a9101d3c',
    wasActually:      'the same warden settle that succeeded 19ms later as entry 346 — a failed retry, not a lost event',
...
function verifyChain(entries, head, expectedCount, opts = {}) {
  ...
  for (const seg of segments) {
    ...
    for (let n = from0; n < seg.rows.length; n++) {
      ...
      // An entry marked c:1 is hashed over canonical bytes and is immune to
      // any storage layer reordering its keys. An unmarked entry predates that
      // fix and is hashed the old way — over whatever order it was written in,
      // which is exactly what a jsonb round-trip destroyed. See CHAIN_GAP.md.
...
/** One honest verdict across both hash eras.
 *
 * Canonical c:1 entries replay directly. Earlier entries were hashed over
 * insertion-ordered JSON before Postgres jsonb reordered their keys, so the
 * public proof page recovers that historical order without rewriting data.
 * A recovered legacy chain is acceptable only when every retained entry
 * reproduces, the claimed head equals the last stored entry, and every missing
 * index is already explicitly disclosed in source. */
function verifyStoredChain(entries, head, expectedCount, opts = {}) {
  const canonical = verifyChain(entries, head, expectedCount, opts);
  if (canonical.ok) return { ...canonical, mode:'canonical' };

  const legacy = verifyLegacy(entries);
  const list = (entries || []).filter(Boolean).slice()
    .sort((a, b) => Number(a.i) - Number(b.i));
  const last = list[list.length - 1] || null;
  const headMatches = !!(head && last && Number(head.i) === Number(last.i)
    && String(head.h) === String(last.h));
  const retainedVerified = legacy.total > 0 && legacy.unrecovered === 0
    && legacy.verified + legacy.orphaned === legacy.total;
  const undisclosed = canonical.undisclosed || [];
  if (headMatches && retainedVerified && undisclosed.length === 0) {
    const missing = canonical.missing || [];
    return {
      ok:true,
      intact:missing.length === 0 && legacy.orphaned === 0,
      mode:'legacy-order-recovery',
      count:list.length,
      brokenAt:null,
      missing,
      undisclosed:[],
      ...
      reason:missing.length
        ? `${legacy.verified} retained entries reproduce; disclosed gap ${missing.join(', ')} leaves ${legacy.orphaned} orphaned segment anchor`
        : `${legacy.verified} retained entries reproduce after recovering legacy JSON key order`,
      legacy:{ total:legacy.total, verified:legacy.verified,
        canonical:legacy.canonical, orphaned:legacy.orphaned,
        unrecovered:legacy.unrecovered },
    };
  }
  return { ...canonical, mode:'failed',
    legacy:{ total:legacy.total, verified:legacy.verified,
      canonical:legacy.canonical, orphaned:legacy.orphaned,
      unrecovered:legacy.unrecovered, headMatches } };
}

module.exports = { append, appendOnce, decideAnchor, verifyChain, logCount, readEntries,
  verifyStoredChain, MEMO_PROGRAM, CHUNK, DISCLOSED_GAPS, canon };
