...
// How the venues actually settle, checked against their own APIs on 2026-08-27:
// Kalshi's crypto series resolve on CF Benchmarks' Real Time Index — "at the last
// minute before expiration, 60 RTI prices are collected, the official value is the
// average"; Polymarket's short-horizon Up/Down markets resolve on a Chainlink 60-second
// TWAP, and say explicitly "not according to any other sources or spot markets".
// This ledger settles everything on a single Pyth print instead, deliberately: three
// venues asking different questions can only be compared by one referee applied
// identically to all of them, including us.
// That choice has an edge, and the edge is where it would do damage. Inside the interval
// a 60-second average could have produced, the verdict is decided by which oracle was
// asked and how it averages — not by the forecast. Those observations are voided rather
// than scored, because scoring them means publishing our own choice of referee as
// somebody else's mistake.
const REFEREE_WINDOW_MS = 60e3;
...
// The referee band (2026-08-27) changed how an observation RESOLVES, so every
// number derived from resolutions was computed under a rule that no longer
// applies and rolls with it. The open observations do not roll: "this venue
// priced this question at p at time t" is raw evidence and is true under either
// rule. Rolling it would throw away the only thing here that was never in doubt.
const K_SCORE  = 'ldg3:score';
const K_RECENT = 'ldg3:recent';
// The drop counters are versioned SEPARATELY from the scores, because they
// answer a different question: not "how did each venue do" but "what does this
// instrument turn away". The moment the sampling rules change, every earlier
// count describes an older instrument. After ranges became resolvable, 388
// 'strike-type-between' drops sat frozen on a public page claiming we refuse
// something we now answer. A stale exclusion is a false statement about the
// method, so the counter resets when the method does — and the reset date is
// published beside it.
const K_DROP   = 'ldg4:dropped';
const DROP_SINCE = '2026-08-27 (reset when the referee band began voiding near-strike observations)';
const MAX_OPEN = 400;
...
async function outcomeOf(o, now) {
  ...
  // Both ends are written inclusive, matching how Kalshi's contiguous buckets read.
  // In practice the boundary itself never reaches this line any more: a strike the
  // settling print lands on sits inside the referee interval by definition and has
  // already voided above — the same answer the on-chain program gives an exact tie.
  const iv = await refereeInterval(o.feed, o.exp, r);
  const ambiguous = s => Number.isFinite(s) && s >= iv.lo && s <= iv.hi;
  if (ambiguous(o.strike) || (o.dir === 'between' && ambiguous(o.strike2)))
    return { status: 'void', reason: 'inside-referee-band' };
...
/** The interval inside which this settlement cannot be told apart from the one a
 *  venue's own 60-second average would have produced. Two sources, unioned:
 *  Pyth's published confidence around the settling print — the oracle's own
 *  statement of what it does not claim to know — and the realised range of our
 *  samples over the final minute, which is the window both venues average over.
 *  A strike inside it is not a forecast we can score. */
async function refereeInterval(feed, exp, r) {
  const conf = Number.isFinite(r.confBps) ? Math.abs(r.price) * (r.confBps / 10_000) : 0;
  let lo = r.price - conf, hi = r.price + conf;
  try {
    const path = await px.pathFor(feed, exp - REFEREE_WINDOW_MS, exp);
    for (const [, v] of (path || [])) {
      if (!Number.isFinite(v)) continue;
      if (v < lo) lo = v;
      if (v > hi) hi = v;
    }
  } catch { /* no path is not a licence to widen or narrow: confidence alone stands */ }
  return { lo, hi, conf };
}
...
module.exports = {
  ...
  REFEREE_WINDOW_MS, refereeInterval,
