...
async function fromKalshi(now) {
  ...
  const obs = [];
  for (const m of rows) {
    const close = asMs(pick(m, 'close_time', 'closeTime', 'expiration_time'));
    if (close == null) { bump('no-expiry'); continue; }
    const dt = close - now;
    if (dt < MIN_HORIZON_MS || dt > MAX_HORIZON_MS) { bump('outside-horizon'); continue; }
    const bid = Number(pick(m, 'yes_bid', 'yesBid'));
    const ask = Number(pick(m, 'yes_ask', 'yesAsk'));
    let p = (Number.isFinite(bid) && Number.isFinite(ask) && (bid || ask))
      ? (bid + ask) / 200                       // cents -> probability
      : Number(pick(m, 'last_price', 'lastPrice')) / 100;
    if (!Number.isFinite(p) || p <= 0 || p >= 1) { bump('no-price'); continue; }
    if (!inBand(p)) { bump('outside-band'); continue; }
    const parsed = parseMarket(pick(m, 'title', 'subtitle', 'ticker'),
      `${pick(m, 'subtitle', 'yes_sub_title') || ''} ${pick(m, 'floor_strike', 'cap_strike') || ''}`);
    if (parsed.drop) { bump(parsed.drop); continue; }
    obs.push({ venue: 'kalshi', id: String(pick(m, 'ticker', 'id')), p: clamp01(p),
      feed: parsed.feed, strike: parsed.strike, dir: parsed.dir, exp: close, at: now });
  }
...
module.exports = {
  ...
  fetchJSON, parseMarket, ASSET_OF, strikeOf, dirOf, inBand, clamp01,
