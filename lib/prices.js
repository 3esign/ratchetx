...
//       upgrade (26 Aug 2026, 16:00 UTC) put it behind a $500/mo plan, and
//       the free Terminal tier has no API access at all. Optional now.
...
// Keyless Hermes still answers until the Core upgrade lands.
const KEY_REQUIRED_YET = Date.now() >= Date.UTC(2026, 7, 26, 16, 0, 0);
...
async function pyth() {
  ...
  if (!r.ok) throw new Error('pyth ' + r.status + (r.status === 401 || r.status === 403
    ? ' — Hermes now requires PYTH_API_KEY (Pyth Core upgrade, 2026-08-26 16:00 UTC)' : ''));
...
const PRICE_TTL_MS = 3000;   // sponsored feeds heartbeat at 60s; 3s is already generous
