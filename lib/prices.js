...
//
//  SOURCE ORDER
//    1. Pyth ON-CHAIN  (lib/onchain_px.js) — sponsored price accounts on
//       Solana mainnet. Free, permissionless, and the exact same accounts
//       our settlement program validates. This is the primary.
//    2. Pyth Hermes    — only if PYTH_API_KEY is set. Sharper, but the Core
//       upgrade requires an API key for direct Hermes access. Optional here
//       because the sponsored on-chain feeds remain the primary route.
...
// Pyth moved the Core cutover to 18 Aug 2026. Do not keep probing an
// authenticated endpoint keylessly after that date.
const KEY_REQUIRED_YET = Date.now() >= Date.UTC(2026, 7, 18, 0, 0, 0);
...
async function pyth() {
  ...
  if (!r.ok) throw new Error('pyth ' + r.status + (r.status === 401 || r.status === 403
    ? ' — Hermes requires PYTH_API_KEY (Pyth Core upgrade, 2026-08-18)' : ''));
...
// and it halves our RPC bill on a free endpoint. Vercel runs many instances,
...
async function fetchPrices() {
  ...
  if (PYTH_KEY || !KEY_REQUIRED_YET) {
    ...
    why.push('hermes: no PYTH_API_KEY');
  }
