...
const { getJSONStrict, getManyJSON, getJSON, setJSON, setJSONEx, acquireLease, releaseLease, applyOnce, hall, hincr } = require('./kv.js');
...
const reportMemo = globalThis.__ratchet_fhreport ||
...
async function noteSettle(feed, kind, eventId = null) {
  if (!feed || !kind) return;
  try {
    if (eventId) await applyOnce(`fhsettle:${kind}:${feed}:${eventId}`,
      { feed, kind, eventId, t:Date.now() },
      { hashKey:HEALTH_H, deltas:{ [`${kind}:${feed}`]:1 } });
    else await hincr(HEALTH_H, `${kind}:${feed}`, 1);
  } catch {}
}
...
async function rowsBetween(from, to) {
  ...
  for (let h = start; h < to + 3600e3; h += 3600e3) {
    keys.push(bucketKey(h));
  }
  const buckets = await getManyJSON(keys);            // throws if the store is unreachable
...
async function report(hours = 24, now = Date.now()) {
  ...
  // A failed counter read must never become a printed zero -- "0 settled" and
  // "we could not ask" are different claims and only one of them is true. The
  // first version enforced that by leaving the read unguarded, which is one
  // option too few: it also meant a broken TELEMETRY counter took down every
  // caller of this report, including /api/game?action=pyth-context, which is
  // the endpoint an agent must read before it can play at all. It did, on
  // 2026-09-03: `g:fh` came back from the Supabase migration as a string
  // instead of a hash, every HGETALL against it threw WRONGTYPE, and the whole
  // context endpoint 500'd because a statistic was unavailable.
  //
  // So there is a third answer, and it is the honest one: say the counters
  // could not be read. `settle` is null rather than zeroed, the reason is
  // published beside it, and nothing that merely wanted a price is taken down
  // by a metric.
  let counters = null, settleUnavailable = null;
  try {
    counters = (await hall(HEALTH_H)) || {};
  } catch (error) {
    settleUnavailable = String((error && error.message) || error);
  }
  let settle = null;
  if (counters) {
    settle = {};
  ...
  return {
    ...
    maxAgeS: MAX_AGE_S,
    feeds, settle, settleUnavailable,
    ...
    limits: [
      ...
      'settle is null, with settleUnavailable saying why, when the counters could not be read. A missing statistic is reported as missing; it is never reported as zero, and it never takes this report down with it.',
    ],
  };
}

/** Shared live projection for agent/UI readers. A caller asking for health
 * reads one cached report; it does not rescan every retained bucket. */
async function cachedReport(hours = 24, now = Date.now()) {
  let H = Math.floor(Number(hours));
  if (!Number.isFinite(H) || H < 1) H = 24;
  H = Math.min(72, H);
  const local = reportMemo.get(H);
  if (local && now - local.cachedAt < 60_000) return local.value;
  const key = `g:fh:live:${H}`;
  const stored = await getJSONStrict(key);
  if (stored && Number.isFinite(Number(stored.cachedAt))
      && now - Number(stored.cachedAt) < 90_000 && stored.value) {
    reportMemo.set(H, stored);
    return stored.value;
  }
  const value = await report(H, now);
  const row = { cachedAt:now, value };
  reportMemo.set(H, row);
  await setJSONEx(key, row, 120);
  return value;
}
...
async function rollupDay(day, now = Date.now()) {
  ...
  const addToIndex = async () => {
    ...
    if (!idx.includes(day)) {
      ...
      await setJSON(DAY_INDEX, idx.slice(-400));
    }
  };
  // A prior process may have written the immutable daily row and died before
  // adding it to the discoverable index.  Repair that partial state instead
  // of making the evidence permanently invisible.
  if (await getJSONStrict(dayKey(day))) {
    const lease = await acquireLease('lock:g:fh:rollup', 60);
    if (!lease) return false;
    try { await addToIndex(); } finally { try { await releaseLease('lock:g:fh:rollup', lease); } catch {} }
    return false;
  }
  ...
  // The old 400-day SET-NX gate was written BEFORE the row.  A process death
  // in between consumed the day forever.  A short ownership-safe lease makes
  // the operation retryable, while the row itself remains the idempotency key.
  const lease = await acquireLease('lock:g:fh:rollup', 60);
  if (!lease) return false;
  try {
    if (await getJSONStrict(dayKey(day))) { await addToIndex(); return false; }
    await setJSONEx(dayKey(day), row, DAY_TTL);
    await addToIndex();
    return true;
  } finally {
    try { await releaseLease('lock:g:fh:rollup', lease); } catch {}
  }
}
...
async function ensureRollups(now = Date.now()) {
  ...
  let made = 0;
  try {
    const days = [1, 2, 3].map(back => dayOf(now - back * 86400e3));
    const [idxRaw, ...rows] = await getManyJSON([DAY_INDEX, ...days.map(dayKey)]);
    const idx = Array.isArray(idxRaw) ? idxRaw : [];
    for (let i = 0; i < days.length; i++) {
      if (rows[i] && idx.includes(days[i])) continue;
      if (await rollupDay(days[i], now)) made++;
    }
...
module.exports = { report, cachedReport, noteSettle, rowsBetween, pct, HEALTH_H,
