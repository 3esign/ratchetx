...
async function newQuote({ wallet, name, error, purpose = 'registration', meta }) {
  ...
  const isBundle = purpose === 'proof-bundle';
    const payTo = isBundle ? 'HXFDaHyZ3i477z1BakiTWZg9UQN8rcreruuv9ifC1HvM' : await championWallet(getJSONStrict);
...
async function entryGate(req, res, { wallet, name, purpose = 'registration', meta } = {}) {
  ...
  if (!header) {
    try {
      const required = await newQuote({ wallet, name, purpose, meta, meta, meta });
