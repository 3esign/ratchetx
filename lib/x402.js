...
async function newQuote({ wallet, name, error, purpose = 'registration' }) {
  ...
  const made = paymentRequired({ id, payTo, amountAtomic: entryAmountAtomic(), feePayer,
    error, purpose });
  await setJSONEx(`x402:q:${id}`, { id, wallet, name, purpose, createdAt: Date.now(),
...
async function entryGate(req, res, { wallet, name, purpose = 'registration' } = {}) {
  if (!enabled()) return null;
  const header = paymentHeader(req);
  if (!header) {
    try {
      const required = await newQuote({ wallet, name, purpose });
...
module.exports = { entryGate, claimGate, consumeEntryClaim, paymentRequired,
  ...
  SOLANA_MAINNET, DEFAULT_FACILITATOR, AGENT_ENTRY_PATH, QUOTE_SECONDS };
