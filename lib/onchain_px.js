...
async function onchainPrices() {
  ...
  for (let i = 0; i < keys.length; i += BATCH) {
    ...
    infos.push(...rows);
    rpcSlots.push(...rows.map(() => Number(res.context && res.context.slot) || 0));
  }
  ...
  syms.forEach((s, i) => {
    ...
    try {
      ...
      const { px, publishTime, prevPublishTime, postedSlot, conf, emaPx, emaConf } =
        decode(inf.data[0], ACCOUNTS[s][1]);
      ...
      out.slots[s] = rpcSlots[i];
      out.postedSlots[s] = postedSlot;
      out.emaPrices[s] = emaPx;
      out.emaConfs[s] = +(emaConf / emaPx * 10000).toFixed(3);
    } catch (e) {
