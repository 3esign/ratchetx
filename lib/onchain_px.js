...
//  Pyth's Core upgrade (26 Aug 2026, 16:00 UTC) puts the Hermes HTTP API
//  behind a paid plan: $500/month minimum. The free Pyth Terminal tier is
//  explicitly "No Pyth API access" — view-only. So the pull-oracle route
//  RATCHET used is no longer free.
//
...
const ENDPOINTS = (process.env.SOLANA_RPC ? [process.env.SOLANA_RPC] : [
...
function decode(b64, wantFeedId) {
  ...
  return { px, publishTime, conf: Number(conf) * 10 ** expo };
}
...
async function onchainPrices() {
  ...
  const out = { src: 'pyth-onchain', ages: {}, confs: {}, pubs: {} };
  const dropped = [];
  ...
  syms.forEach((s, i) => {
    ...
    try {
      ...
      const { px, publishTime, conf } = decode(inf.data[0], ACCOUNTS[s][1]);
      const age = now - publishTime;
      if (age > MAX_AGE_S) throw new Error(`stale ${age}s`);
      out[s] = px;
      out.ages[s] = age < 0 ? 0 : age;
      out.confs[s] = px > 0 ? +((conf / px) * 10000).toFixed(3) : null;   // bps
...
module.exports = { onchainPrices, ACCOUNTS, MAX_AGE_S, ENDPOINTS, PYTH_OWNERS, decode };
