...
async function buildAgentReport(id) {
  ...
  const proofPage = shotId => `https://ratchetx.xyz/api/shot?w=${encodeURIComponent(identity)}&id=${encodeURIComponent(shotId)}`;
  // The public proof handler reads retained closed shots, not compact history.
  // Do not advertise an open shot or a history-only row as an available page.
  const proofShots = rich.filter(row => row && ['hit','miss','void'].includes(row.res)
    && /^[a-z0-9]{4,16}$/i.test(String(row.id || '')));
  return { status:200, body:{ ok:true, v:RELEASE, reportCard:{
    identity,
    links:{ report:`https://ratchetx.xyz/api/agent?id=${encodeURIComponent(identity)}`,
      settledShots:proofShots.map(row => ({ shotId:row.id, proofPage:proofPage(row.id) })) },
    ...
    latestReceipt:{ ...(latest ? { shotId:latest.shotId, digest:latest.digest,
      ...
      proofPage:proofPage(latest.shotId),
      proofPageAvailable:proofShots.some(row => row.id === latest.shotId),
      ...
      receiptType:'retained-evidence-agent-run',
      receiptStatus:latest ? 'available' : 'not-found-in-retained-history',
      explanation:latest
        ? 'A stored AgentRun audit receipt was found for a shot in retained player history. It is not evidence of HTTP session replay or proof-bundle payment.'
        : 'No stored AgentRun audit receipt was found for shots in retained player history. This does not mean a shot failed or HTTP session replay was attempted or failed.',
      httpSessionReplayStatus:'not-assessed' },
  } } };
}
