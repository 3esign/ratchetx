...
async function incrFloat(key, by, exSeconds = null) {
  drop(key);
  // Only send p_ex_seconds when set: production still runs 001's two-argument
  // ratchet_kv_incr until 003 is applied, and PostgREST matches by named args —
  // sending the extra argument unconditionally would fail every deposit-queue
  // increment on the live site.
  const args = { p_key:String(key), p_by:Number(by) };
  if (exSeconds != null) args.p_ex_seconds = exSeconds;
  return Number(await rpc('ratchet_kv_incr', args));
}
