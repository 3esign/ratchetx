...
async function sample(prices) {
  ...
  try {
    ...
    await setJSONEx(LATEST_POLL_KEY, {
...
async function latestSnapshot(now = Date.now()) {
  const [poll, ...streamRows] = await getManyJSON(
    [LATEST_POLL_KEY, ...FEEDS.map(f => `pxlatest:${f}`)]);
  const feeds = {};
  for (let i = 0; i < FEEDS.length; i++) {
    const feed = FEEDS[i];
    const candidates = [];
    const mapValue = map => optionalNumber(map && map[feed]);
    const pollPrice = mapValue(poll && poll.prices);
    if (pollPrice != null && pollPrice > 0) {
      candidates.push({
        observedAt:optionalNumber(poll.observedAt),
        source:String(poll.source || 'pyth-onchain-poll'),
        price:pollPrice,
        publishTime:mapValue(poll.publishTimes),
        prevPublishTime:mapValue(poll.prevPublishTimes),
        confidenceBps:mapValue(poll.confidenceBps),
        ageAtObservationS:mapValue(poll.ages),
        rpcSlot:mapValue(poll.rpcSlots),
        postedSlot:mapValue(poll.postedSlots),
        emaPrice:mapValue(poll.emaPrices),
        emaConfidenceBps:mapValue(poll.emaConfidenceBps),
      });
    }
    const streamPrice = optionalNumber(streamRows[i] && streamRows[i].price);
    if (streamPrice != null && streamPrice > 0)
      candidates.push({ ...streamRows[i], price:streamPrice });
    candidates.sort((a, b) => (Number(b.publishTime) || 0) - (Number(a.publishTime) || 0)
      || (Number(b.postedSlot) || 0) - (Number(a.postedSlot) || 0)
