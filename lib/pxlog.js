...
//  property — the exit PRICE stops depending on who triggers the settle. Not
//  whether it settles: a shot whose crossing nobody recorded voids and refunds,
//  which is a different outcome from the one the price would have given. The
//  rule removes the choice of number, not the choice of showing up.
//
...
// ---------------------------------------------------------------------------
//  BUCKET READS ARE THE EXPENSIVE ONES, AND MOST OF THEM ARE FOR HOURS THAT
//  CANNOT CHANGE AGAIN.
//
//  Measured 2026-09-03 from a full copy of the store: `pxu:` is 98 rows and
//  15.88 MB, `px:` is 98 rows and 5.60 MB -- about 162 KB and 57 KB per hour
//  respectively, and together 69% of the entire database. Every settlement
//  walks three hours of BOTH, so one priceCrossing costs roughly 600 KB. The
//  client re-triggers that walk on every poll while a shot is expiring. That,
//  not the state poll's two dozen small reads, is where 25.9 GB of egress went
//  against a 5 GB allowance.
//
//  A bucket for an hour that has already ended never changes: samples are only
//  ever appended to the CURRENT hour. So a closed bucket can be held for the
//  life of the instance, and the open one for a few seconds. Nothing about
//  settlement semantics moves -- the same bytes are compared, they are simply
//  not fetched forty times.
//
//  Two rules keep this honest. Only a read that SUCCEEDED is remembered: a
//  store hiccup must never harden into "no samples in that hour" and void a
//  valid shot. That is why these go through getJSONStrict, whose contract is
//  that a failed read throws rather than returning nothing -- so a null from it
//  is the store's answer, not its silence, and is safe to keep. And every write
//  to a bucket drops its entry, so an instance can never settle from a copy it
//  has already replaced.
// ---------------------------------------------------------------------------
//  A CLOSED HOUR is immutable and cached without a clock. THE OPEN HOUR is
//  cached for five seconds, and that window rests on two things, not one.
//
//  The first is the settlement rule: the crossing is the EARLIEST validated
//  transition at or after expiry, so a row this instance has not seen yet can
//  only delay a settlement, never move one -- a later row is never chosen over
//  an earlier one.
//
//  The second is that a write to a bucket must drop it here, and that has to be
//  true of EVERY write, not only the ones this module makes. It was not: the
//  drop lived in pxlog's own write paths, so a sample written straight through
//  kv.setJSON stayed invisible for five seconds. test_calibration found it --
//  eleven assertions about scored shots, red -- because within one process a
//  sample that exists but is not visible is a sample that does not exist. The
//  invalidation now lives in lib/kv.js's `rdrop`, which every write already
//  calls, so the memo cannot outlive a write through any door.
const bucketMemo = globalThis.__ratchet_bucketmemo
  || (globalThis.__ratchet_bucketmemo = new Map());
const BUCKET_MEMO_MAX = 96;
const OPEN_BUCKET_TTL_MS = 5_000;
const bucketHourOf = key => Date.parse(key.replace(/^(px|pxu):/, '') + ':00:00Z');
const bucketIsClosed = (key, now) => {
  const hour = bucketHourOf(key);
  return Number.isFinite(hour) && hour + 3600_000 <= now;
};
const dropBucket = key => { if (bucketMemo.size) bucketMemo.delete(key); };

// A cache that outlives its store will lie. In demo/test mode the backing store
// is a Map on globalThis, and replacing that Map replaces the world -- so a memo
// built against the old one must not answer questions about the new one. Caught
// by test_warden, which measures two different markets by wiping the store
// between them and would otherwise have read the first market's buckets back
// out of this cache. Durable backends never swap the reference, so this costs
// them one identity comparison and nothing else.
let memoStore = globalThis.__ratchet_mem;
const bucketMemoForCurrentStore = () => {
  if (memoStore !== globalThis.__ratchet_mem) { bucketMemo.clear(); memoStore = globalThis.__ratchet_mem; }
  return bucketMemo;
};

async function readBucket(key, now = Date.now()) {
  const hit = bucketMemoForCurrentStore().get(key);
  if (hit && (bucketIsClosed(key, now) || now - hit.t < OPEN_BUCKET_TTL_MS)) return hit.v;
  // If this throws, nothing is cached and the caller sees the failure: an
  // unreadable store must stay unreadable rather than become an empty hour.
  const value = await getJSONStrict(key);
  if (bucketMemo.size >= BUCKET_MEMO_MAX) bucketMemo.delete(bucketMemo.keys().next().value);
  bucketMemo.set(key, { v: value, t: now });
  return value;
}

async function readBuckets(keys, now = Date.now()) {
  bucketMemoForCurrentStore();
  const missing = keys.filter(key => {
    const hit = bucketMemo.get(key);
    return !(hit && (bucketIsClosed(key, now) || now - hit.t < OPEN_BUCKET_TTL_MS));
  });
  if (missing.length) {
    const fetched = await getManyJSON(missing);
    ...
    // a key that fetched null is answered from `fetched`, never from the memo
    return keys.map(key => {
      const index = missing.indexOf(key);
      return index >= 0 ? fetched[index] : bucketMemo.get(key).v;
    });
  }
  return keys.map(key => bucketMemo.get(key).v);
}

// Every feed the price log records, and so every feed a shot can settle
// against by first crossing. Stocks are here because settlement reads this row.
const FEEDS = ['SOL', 'BTC', 'ETH', 'BONK', 'WIF', 'JUP', 'PUMP',
               'TSLA', 'NVDA', 'PLTR', 'COIN', 'HOOD'];
// The subset the websocket heartbeat can subscribe to. Pyth publishes a
// sponsored on-chain account for the crypto majors and none at all for an
// equity index feed, so a stock can never appear in the stream -- not as a
// fault, but by construction. Measuring stocks against a capture path that
// structurally cannot carry them would report `degraded` forever, and the
// whole point of the note above streamHealth is that an amber which is always
// on gets ignored. The stream alarm therefore watches only what can stream.
const STREAM_FEEDS = ['SOL', 'BTC', 'ETH', 'BONK', 'WIF', 'JUP', 'PUMP'];
...
async function ingestUpdate(feed, update) {
  ...
  try {
    ...
    if (!duplicate) {
      ...
      // AFTER the write, for the reason spelled out at the poll site below.
      dropBucket(key);
    }
...
async function sample(prices) {
  ...
  try {
    ...
    rows.push(row);
    await setJSONEx(key, rows, BUCKET_TTL);
    // Dropped AFTER the write, never before: between a drop and a write another
    // read could re-cache exactly the copy we are replacing.
    dropBucket(key);
...
async function priceAt(ts, now = Date.now(), source = null) {
  ...
  for (let h = ts; h <= deadline + 3600e3; h += 3600e3) {
    ...
    const rows = await readBucket(bucketKey(h), now);
    if (rows && rows.length) {
      for (const r of rows) {
...
async function priceCrossing(feed, ts, now = Date.now(), source = 'pyth-onchain') {
  ...
  for (let h = ts - 3600e3; h <= deadline + 3600e3; h += 3600e3) {
    const [sampleRows, updateRows] = await Promise.all([
      readBucket(bucketKey(h), now), readBucket(updateBucketKey(h), now),
    ]);
...
async function pathFor(feed, from, to, source = null) {
  ...
  const buckets = await readBuckets(keys);
...
async function streamHealth(now = Date.now()) {
  const rows = await getManyJSON(STREAM_FEEDS.map(f => `pxstream:${f}`));
  ...
  return { active, usable, total:STREAM_FEEDS.length, ok:active === STREAM_FEEDS.length,
...
module.exports = { sample, ingestUpdate, priceAt, priceCrossing, pathFor, evidencePathFor,
  ...
  SAMPLE_MS, FEEDS, STREAM_FEEDS, LATEST_POLL_KEY };
