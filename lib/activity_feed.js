...
function rowFromEvent(entry) {
  ...
  const row = {t:entry.t,w:shortW(e.w),actorWallet:e.w,source:'event-log',logIndex:entry.i};
...
// Strictly read-only inspection, including before migration. It neither acquires
// a lease nor persists the preview. Useful to consumers who need activity without
// invoking the game's lazy state/settlement endpoint. Preview scans are memoized.
let preview = null;
async function peekFeed() {
  const current = await kv.getJSONStrict(KEY);
  if (Array.isArray(current)) return {feed:mergeRows(current),
    projection:{persisted:true,limit:LIMIT,scope:'player-activity'}};
  if (!preview || Date.now()-preview.t>=30000) {
    const rows = await loadUnderLease(); // this helper reads only; its caller normally holds a lease
    preview = {t:Date.now(),rows};
  }
  return {feed:preview.rows,projection:{persisted:false,limit:LIMIT,
    scope:'player-activity',previewedAt:preview.t,recoveryWindow:RECOVERY_WINDOW}};
}
...
module.exports = {readFeed,peekFeed,bumpFeed,rowFromEvent,mergeRows,KEY,LIMIT,RECOVERY_WINDOW};
