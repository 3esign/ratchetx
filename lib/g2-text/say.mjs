...
export function sentenceForResult(snapshot, options = {}) {
  ...
  if (snapshot.checks?.resultOutcomeAndXp === true && snapshot.checks?.gameResultHash === true && snapshot.reconstruction?.result)
    return sentenceFor(snapshot.reconstruction.result, options);
...
export function saveReaderResult(snapshot, { rpc = 'https://api.devnet.solana.com', feed = null } = {}) {
  ...
  const reconstructed = snapshot.checks?.resultOutcomeAndXp === true && snapshot.checks?.gameResultHash === true && snapshot.reconstruction;
  return jsonSafe({ format: 'ratchetx-g2-archive', format_version: 1,
    verify: { cluster: meta.cluster, command: `solana confirm ${signature} --url ${rpc} --verbose --output json`,
      note: reconstructed ? 'This command inspects the terminal transaction. The bundled permanent account bytes reconstruct both prices, outcome and XP and match the game-result and archive-row hashes. Ledger balances, full history, cryptographic transaction signatures and validator finality were not replayed. The Shot account is closed.' : 'This command inspects the terminal transaction. Core log provenance was checked; economics and the HistoryPage commitment were not replayed. The Shot account is closed.' },
    ...
    reconstruction: reconstructed || null, verificationError: snapshot.verificationError || null,
    proofMaterialMeaning: snapshot.receipt.event.result.state === 4 ? 'revealed salt' : 'commitment',
    omitted: reconstructed ? ['resolver and resolution slot', 'raw closed Shot account bytes', 'complete HistoryPage fold and ledger balance replay'] : ['entry and exit price/confidence/exponent/publish time', 'hit and XP', 'resolver and resolution slot'],
