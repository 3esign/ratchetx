...
try {
  ...
  if (/^\/proof\/?$/.test(location.pathname)) {
    // /proof?player=<base58>&nonce=<n>[&terminal=<sig>] is the receipt room for ONE shot (the links the
    // agent replies carry); without those it opens the shared devnet run.
    const q = new URLSearchParams(location.search), qp = q.get('player'), qn = q.get('nonce');
    if (qp && qn && /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(qp) && /^[0-9]{1,20}$/.test(qn)) {
      run(async () => { const selection = { player: qp, nonce: qn }; if (q.get('terminal')) selection.terminalSignature = q.get('terminal');
        const result = await readSelection(selection); await presentShot(result, selection);
        status(result.kind === 'unavailable' ? 'No live Shot or terminal receipt is readable for that player and nonce yet.' : 'Showing the receipt for ' + qp.slice(0, 6) + '… shot #' + qn + '.'); });
    } else { $('watch').click(); }
