...
function renderSaved() {
  ...
  for (const record of records) {
    ...
    button.onclick = () => run(async () => { const out = await game.reconcile({ nonce: record.nonce }); showShot(out.result, { player: provider.publicKey.toBase58(), nonce: record.nonce, terminalSignature: game.savedGames().find(r => r.nonce === record.nonce)?.terminalSignature }); renderSaved(); status(out.pending ? 'The transaction is still unresolved. Your saved key and signature are retained.' : 'Shot read from devnet.'); }); root.append(button);
  }
}
...
async function refreshCurrentShot() {
  ...
  else result = await game.readShot(currentShot);
  showShot(result, currentShot); renderSaved();
}
async function refresh() {
  ...
  const claimed = value.ledger && (value.ledger.legacyCredits > 0n || value.ledger.legacyXp > 0n); $('claim').hidden = !value.allocation || claimed;
  $('allocation-note').textContent = !value.player ? 'Connect to read your on-chain ledger.' : claimed ? 'Your test allocation is already claimed. Credits shown above come from your devnet ledger.' : value.allocation ? 'This wallet can claim ' + BigInt(value.allocation.credits).toLocaleString('en-US') + ' test credits.' : 'This wallet has no test allocation in the registered devnet economy. You can still inspect the shared run.';
...
$('seal').onclick = () => run(async () => { if(config.admission?.enabled !== true) throw new Error(config.admission?.reason || 'New shots are not open yet'); const out = await game.seal({ side: selectedSide, pBps: Number($('confidence').value) * 100, stake: $('stake').value.trim(), expectedTargets: context?.timing }); const nonce = String(out.result.shot.nonce); await refresh(); showShot(out.result, { player: provider.publicKey.toBase58(), nonce }); status('Shot #' + nonce + ' sealed. Keep the reveal key and return when the chain is ready for reveal.'); });
$('watch').onclick = () => run(async () => { const selection = { player: config.watchPlayer, nonce: config.watchNonce }; const terminal = new URLSearchParams(location.search).get('terminal'); if (terminal) selection.terminalSignature = terminal; const result = await game.readShot(selection); showShot(result, selection); status(result.kind === 'unavailable' ? 'The shared shot has no available live account or terminal receipt yet.' : 'Showing the funded devnet wallet; it is separate from your connected wallet.'); });
$('restore-key').onchange = () => run(async () => { const file = $('restore-key').files[0]; if (!file) return; if (file.size > 12000) throw new Error('This reveal-key file is too large'); const out = await game.importRevealKey(await file.text()); renderSaved(); showShot(out.result, { player: provider.publicKey.toBase58(), nonce: out.nonce }); status('Reveal key restored and matched to the live Shot.'); $('restore-key').value = ''; });
...
try {
  ...
  await refresh(); status('Registered devnet rules loaded. Connect to play, or inspect the shared devnet run.');
