...
$('watch').onclick = () => run(async () => { const selection = { player: config.watchPlayer, nonce: config.watchNonce }; const terminal = new URLSearchParams(location.search).get('terminal'); if (terminal) selection.terminalSignature = terminal; const result = await readSelection(selection); showShot(result, selection); status(result.kind === 'unavailable' ? 'The shared shot has no available live account or terminal receipt yet.' : 'Showing the funded devnet wallet; it is separate from your connected wallet.'); });
