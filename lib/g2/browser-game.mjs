...
export function walletKeptOurTransaction(original, signed, { ourPrograms = [] } = {}) {
  ...
  for (let i = 0; i < ours.length; i++) if (JSON.stringify(ours[i]) !== JSON.stringify(theirs[i])) return false;
  const forbidden = new Set(ourPrograms.map(String));
  return theirs.slice(ours.length).every(ix => !forbidden.has(ix.program) && ix.program !== '11111111111111111111111111111111');
}
