...
// Mirrors DEVNET_PLAYGROUND_CREDITS in the program; the chain is the authority, this is the read-back expectation.
export const DEVNET_PLAYGROUND_CREDITS = 10_000n;
...
// Wallets such as Phantom may append their own guard instructions (Lighthouse) to a
// transaction before signing, so byte-equality of the message is the wrong test. What
// must hold: same fee payer, no address-table lookups, every instruction we built is
// still there, unchanged and first, and anything appended is not aimed at our programs
// and adds no signer. The grant flow already accepts wallets this way; this is the same rule.
function decodeInstructions(message) {
  const keys = message.staticAccountKeys.map(k => k.toBase58());
  return message.compiledInstructions.map(ix => ({ program: keys[ix.programIdIndex],
    accounts: ix.accountKeyIndexes.map(i => keys[i]), data: Array.from(ix.data, b => b.toString(16).padStart(2, '0')).join('') }));
}
export function walletKeptOurTransaction(original, signed, { ourPrograms = [] } = {}) {
  if (!signed?.message || (signed.message.addressTableLookups?.length || 0) !== 0) return false;
  const a = original.staticAccountKeys[0]?.toBase58(), b = signed.message.staticAccountKeys[0]?.toBase58();
  if (!a || a !== b || signed.message.header.numRequiredSignatures !== original.header.numRequiredSignatures) return false;
  const ours = decodeInstructions(original), theirs = decodeInstructions(signed.message);
  // Wallets prepend ComputeBudget (Phantom sets a unit limit/price) and append guards
  // (Lighthouse). Neither may touch our programs or move lamports; whatever remains after
  // removing those must be exactly our instructions, in order.
  // Only these two wallet helpers are tolerated; any other foreign instruction, before or
  // after ours, is a different transaction. Phantom: ComputeBudget + Lighthouse.
  const WALLET_HELPERS = new Set(['ComputeBudget111111111111111111111111111111', 'L2TExMFKdjpN9kozasaurPirfHy9P8sbXoAN1qA3S95']);
  const forbidden = new Set(ourPrograms.map(String));
  const kept = [];
  for (const ix of theirs) {
    const isOurs = ours[kept.length] && JSON.stringify(ours[kept.length]) === JSON.stringify(ix);
    if (isOurs) { kept.push(ix); continue; }
    if (forbidden.has(ix.program) || !WALLET_HELPERS.has(ix.program)) return false;
    if (kept.length > 0 && kept.length < ours.length) return false; // helper wedged between ours
  }
  return kept.length === ours.length;
}
...
export function createBrowserGame({ web3, connection, config, wallet = null,
  ...
  // A wallet that has never touched devnet has no lamports, and the RPC answers a claim
  // with the unhelpful "AccountNotFound". Say what is true instead, and try the free
  // devnet faucet once on the player's behalf - it needs no key and may be rate limited.
  const FEE_FLOOR_LAMPORTS = 3_000_000; // rent for a ledger plus fees; 0.003 SOL
  async function ensureDevnetSol(player) {
    let balance = await connection.getBalance(player, 'confirmed');
    if (balance >= FEE_FLOOR_LAMPORTS) return balance;
    notify('funding');
    try {
      const signature = await connection.requestAirdrop(player, 50_000_000);
      const block = await connection.getLatestBlockhash('confirmed');
      await connection.confirmTransaction({ signature, ...block }, 'confirmed');
      balance = await connection.getBalance(player, 'confirmed');
    } catch { /* faucet refused; the sentence below says so */ }
    requireFact(balance >= FEE_FLOOR_LAMPORTS, 'This wallet has no devnet SOL to pay rent and fees (about 0.003 SOL). The public faucet refused an airdrop just now; get devnet SOL at faucet.solana.com for ' + player.toBase58() + ' and try again.');
    return balance;
  }

  async function send(instructions, player, onBroadcast = () => {}) {
    await boundary(); sameWallet(player);
    await ensureDevnetSol(player);
    const block = await connection.getLatestBlockhash('confirmed');
    ...
    sameWallet(player);
    requireFact(signed?.message && (equal(originalMessage, signed.message.serialize()) || walletKeptOurTransaction(transaction.message, signed, { ourPrograms: [config.programs.core, config.programs.timepin] })), 'Wallet returned a different transaction');
  ...
  const claim = () => exclusive(async () => {
    ...
    // A wallet named in the migration snapshot claims its exact allocation; every other
    // wallet takes the devnet playground door, which the program opens only on the
    // devnet-genesis economy. Both land in the legacy bucket and both are once per wallet.
    const allocation = before.allocation;
    const credits = allocation ? exact(allocation.credits, 'credits') : DEVNET_PLAYGROUND_CREDITS;
    const xp = allocation ? exact(allocation.xp, 'XP') : 0n;
    const instruction = allocation
      ? actions.claimLegacyIx({ economy: before.economy, player, credits, xp, proof: allocation.proof.map(p => bytes(p, 'claim proof')) })
      : actions.claimDevnetCreditsIx({ economy: before.economy, player });
  ...
  // salt: optional 32 bytes supplied by a stateless caller (a seed-derived agent) so the reveal
  // material can be re-derived later; browsers leave it undefined and get a random one.
  const seal = ({ side, pBps, stake, expectedTargets, salt: givenSalt }) => exclusive(async () => {
    ...
    const salt = existing ? bytes(existing.salt, 'saved reveal key') : givenSalt ? bytes(givenSalt, 'salt') : cryptoImpl.getRandomValues(new Uint8Array(32));
