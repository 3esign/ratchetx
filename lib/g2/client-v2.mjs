...
export const CORE_INSTRUCTION = Object.freeze({
  ...
  claim_devnet_credits: '843d144331190272',
...
export const TIMEPIN_INSTRUCTION = Object.freeze({
  ...
  hold_need: '2d35a819c9fb5376',
  release_hold: '6a6d46a2c59e5cf3',
  close_need: 'fadd9352bde43133',
...
export function createCoreG2Client({
  ...
  // One game's claim on one target's evidence. Sealing creates two of these,
  // and they are what finally makes TimepinNeedV2.open_refs a real count - the
  // field was declared in the first version and incremented by nothing, which
  // is why the evidence rent never came back.
  const needHoldPda = (need, holder) => derive(timepinProgram, [
    utf8('hold'), pkBytes(need, 'need'), pkBytes(holder, 'holder'),
  ]);
  ...
  // Rent that comes back. hold_need is what Core CPIs at seal; the other two
  // are maintenance any operator can perform, and a keeper that never calls
  // them never gets its capture rent back.
  const holdNeedIx = ({ actor, specHash, targetTs, holder }) => {
    const exact = hash32(specHash, 'specHash');
    const [need] = needPda(exact, targetTs);
    const [hold] = needHoldPda(need, holder);
    return instruction(timepinProgram, 'hold_need', [
      meta(actor, true, true), meta(need, false, true), meta(holder),
      meta(hold, false, true), meta(SystemProgram.programId),
    ]);
  };

  const releaseHoldIx = ({ actor, specHash, targetTs, holder, rentRefund }) => {
    const exact = hash32(specHash, 'specHash');
    const [need] = needPda(exact, targetTs);
    const [hold] = needHoldPda(need, holder);
    return instruction(timepinProgram, 'release_hold', [
      meta(actor, true, true), meta(need, false, true), meta(holder),
      meta(hold, false, true), meta(rentRefund, false, true),
    ]);
  };

  // `candidate` is the captured price for this target. A Need that expired
  // unanswered has none, and the caller says so by passing the Need itself -
  // which is also why candidateRentRefund is then ignored.
  const closeNeedIx = ({ actor, specHash, targetTs, needRentRefund, candidate = null, candidateRentRefund = null }) => {
    const exact = hash32(specHash, 'specHash');
    const [need] = needPda(exact, targetTs);
    const candidateKey = candidate === null ? need : pk(candidate);
    const candidateRefund = candidateRentRefund === null ? need : pk(candidateRentRefund);
    return instruction(timepinProgram, 'close_need', [
      meta(actor, true, true), meta(need, false, true),
      meta(needRentRefund, false, true),
      meta(candidateKey, false, true), meta(candidateRefund, false, true),
    ]);
  };
  ...
  const sealForwardIx = async ({
    ...
    return instruction(coreProgram, 'seal_forward', [
      ...
      // The Needs are writable now: the seal takes a hold on each, which moves
      // their open_refs.
      meta(entryNeed, false, true), meta(exitNeed, false, true),
      meta(needHoldPda(entryNeed, shot)[0], false, true),
      meta(needHoldPda(exitNeed, shot)[0], false, true),
      meta(timepinProgram),
      meta(SystemProgram.programId),
  ...
  return Object.freeze({
    ...
    needHoldPda, holdNeedIx, releaseHoldIx, closeNeedIx,
