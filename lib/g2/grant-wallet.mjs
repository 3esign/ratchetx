...
export function createGrantWallet({web3,connection,config,wallet,storage=globalThis.localStorage,cryptoImpl=globalThis.crypto,onStatus=()=>{}}){
 ...
 async function approve(plan){const m=plans.get(plan);assert(m,'Review this permission before approving');await boundary(m.identity);
  ...
  assert(signed?.message&&same(signed.message.serialize(),m.message),'Wallet changed the reviewed permission');assert(signed.signatures?.length===1&&signed.signatures[0]?.length===64&&signed.signatures[0].some(Boolean),'Owner signature is missing');
  const signerKey=await cryptoImpl.subtle.importKey('raw',owner().toBytes(),{name:'Ed25519'},false,['verify']);
  assert(await cryptoImpl.subtle.verify('Ed25519',signerKey,signed.signatures[0],m.message),'Returned owner signature is invalid');
  const signature=base58(signed.signatures[0]);const record={...storageProbe,state:'pending',signature,blockhash:m.blockhash,lastValidBlockHeight:m.lastValidBlockHeight,limits:m.limits};save(m.identity,record);plans.delete(plan);
  try{
   ...
   onStatus({phase:'confirming',signature});const confirmation=await connection.confirmTransaction({blockhash:m.blockhash,lastValidBlockHeight:m.lastValidBlockHeight,signature},'confirmed');assert(confirmation?.value&&confirmation.value.err===null,'Permission transaction failed');
