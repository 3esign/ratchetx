...
export function createAgentSession({ web3, connection, config, session, signer,
  ...
  need(!player.equals(delegate), 'DISTINCT_LOCAL_DELEGATE_REQUIRED');
  ...
  const read = async record => {
    ...
    if (snapshot.kind === 'account') {
      need(equal(snapshot.shot.commit, bytes(record.commitment, 32)) && snapshot.shot.stake === BigInt(record.stake)
        && new web3.PublicKey(snapshot.shot.delegate).equals(delegate) && new web3.PublicKey(snapshot.shot.rentRefund).equals(delegate)
    ...
    if (snapshot.kind === 'archive') {
      ...
      need(equal(archivedCommit, bytes(record.commitment, 32)) && result.stake === BigInt(record.stake)
        && new web3.PublicKey(result.delegate).equals(delegate) && result.entryTargetTs === BigInt(record.entryTargetTs)
  ...
  async function reconcileRecord(record, save) {
    ...
    if (tx) {
      ...
      if ((status?.err && status.confirmationStatus === 'finalized') || (!status && expired)) {
        const context = await grants.readGrant(identity);
  ...
  async function execute(record, state, save) {
    ...
    const context = await grants.readGrant(identity);
    need(context.kind === 'account' && context.grant, 'OWNER_GRANT_REQUIRED');
    ...
    const intent = resolveIntent(record.text, {
      ...
      context: { feeds: [] }, limits: { maxStakeCredits: decimal(context.grant.maxStake), maxGrossCredits: decimal(context.grant.maxGrossStake) },
      session: { grossCredits: decimal(context.grant.grossStakeUsed) }, player: { credits: decimal(context.ledger.credits) },
    });
    need(!intent.resolution.notes.some(note => note.startsWith('requested ') && note.includes('horizon unavailable')), 'TARGET_UNAVAILABLE');
    const stake = BigInt(intent.stake), permit = grants.grantAllows({ grant: context.grant, economy: context.economy, nowTs: context.chainNowTs, stake });
    need(permit.ok, 'GRANT_LIMIT_REFUSED');
    ...
    const instructions = [client.openHistoryPageIx({ actor: delegate, player, economyHash, pageIndex: plan.pageIndex }),
  ...
  const runCommand = ({ commandId: id, text } = {}) => locked(async (state, save) => {
    ...
    try {
      ...
      if (kind === 'meta') response = publicReply('META', 'That is a skill command; no prediction was sent.\nInstall or update the G2 skill: https://github.com/3esign/ratchetx/tree/main/skills/ratchetx-g2');
        ...
        else { const context = await grants.readGrant(identity); response = publicReply('STATUS', 'Test credits: ' + (context.ledger?.credits ?? 0n) + '. No saved prediction yet.'); }
      } else if (kind === 'board') { const context = await grants.readGrant(identity); response = publicReply('BOARD', 'SOL higher or lower over ' + context.ruleset.args.horizonSeconds / 60 + ' min. Pyth prices.\nPlay: "@bankrbot play"'); }
      ...
      const allowed = ['OWNER_GRANT_REQUIRED', 'NO_TEST_CREDIT_ALLOCATION', 'ASSET_NOT_ON_BOARD', 'ASSET_AMBIGUOUS',
        'TARGET_UNAVAILABLE', 'PLAY_INTENT_REQUIRED', 'GRANT_LIMIT_REFUSED', 'PRIOR_NONCE_UNRESOLVED', 'ADMISSION_CLOSED', 'LOCAL_DELEGATE_SIGNER_REQUIRED'];
  ...
  const reveal = ({ commandId: id } = {}) => locked(async (state, save) => {
    ...
    need(new web3.PublicKey(snapshot.shot.delegate).equals(delegate) && new web3.PublicKey(snapshot.shot.rentRefund).equals(delegate), 'FROZEN_DELEGATE_MISMATCH');
    ...
    const clockContext = await grants.readGrant(identity);
