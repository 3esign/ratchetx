...
export function createRevealedArchiveVerifier({ client, web3, cryptoImpl = globalThis.crypto }) {
  ...
  async function authenticateCandidate(spec, need, account) {
    ...
    // 151 since 2026-09-10: CandidateV2 carries rent_payer, a 32-byte field after
    // `need`, so the capture rent has an address to be returned to. Every offset
    // below moved with it.
    const raw = bytes(account.info.data, 151, 'CandidateV2 account');
