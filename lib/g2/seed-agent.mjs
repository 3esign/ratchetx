...
export function createSeedAgent({ web3, connection, config, seed, onStatus = () => {} }) {
  ...
  // A closed game's receipt is only a claim until the archive is rebuilt and checked; the
  // reply formatter refuses to name HIT/MISS otherwise. Same step the journaled runner takes.
  const verified = async snapshot => snapshot.kind === 'archive' ? reconstructArchiveSnapshot({ snapshot, connection, client: game.client, web3, cryptoImpl: webcrypto }) : snapshot;
  const withFooter = async snapshot => ({ ...formatG2Reply(await verified(snapshot)), mode: 'seed-self-play', player: player.toBase58() });
  ...
  async function advance(before) {
    ...
    if (snapshot.kind === 'archive') return { open: false, snapshot, nonce, response: await withFooter(snapshot) };
    ...
    if (snapshot.shot.state === 3) {
      ...
      return { open: false, revealedNow: true, snapshot: out.result, nonce, response: await withFooter(out.result) };
    }
    return { open: [1, 2, 7].includes(snapshot.shot.state), snapshot, nonce, response: await withFooter(snapshot) };
  }
  ...
  async function play(text) {
    ...
    onStatus({ phase: 'play-load' });
    let view = await game.load();
    let latest = null;
    if (Number(view.ledger?.open || 0) > 0) {
      onStatus({ phase: 'play-advance' });
      latest = await advance(view);
    }
    ...
    if (!view.ledger || view.ledger.credits < view.economy.args.minStake) {
      ...
      onStatus({ phase: 'claiming' }); await game.claim(); onStatus({ phase: 'play-reload' }); view = await game.load();
    }
    const intentFor = current => {
      const horizon = Number(current.ruleset.args.horizonSeconds) / 60;
      return resolveIntent(text, {
        board: { targets: [{ id: rulesetHash, kind: 'dir', feed: 'SOL', mins: horizon }], stakeRule: { max: decimal(current.economy.args.maxStake) } },
        context: { feeds: [] }, limits: { maxStakeCredits: decimal(current.economy.args.maxStake), maxGrossCredits: decimal(current.ledger.credits) },
        session: { grossCredits: '0' }, player: { credits: decimal(current.ledger.credits) },
      });
    };
    const sealFor = async current => {
      onStatus({ phase: 'play-intent' });
      const intent = intentFor(current);
      const side = intent.side === 'YES' ? 1 : 0, pBps = Math.round(intent.p * 10000), stake = String(intent.stake);
      const nonce = current.ledger.nextShotNonce, salt = seedSalt(seed, player.toBase58(), nonce, side, pBps);
      onStatus({ phase: 'play-seal' });
      return { nonce, out: await game.seal({ side, pBps, stake, expectedTargets: current.timing, salt: hex(salt) }) };
    };
    let sealed;
    try { sealed = await sealFor(view); }
    catch (error) {
      if (!/target grid moved/i.test(error?.message || '')) throw error;
      onStatus({ phase: 'play-reload' });
      view = await game.load();
      sealed = await sealFor(view);
    }
    const { nonce, out } = sealed;
    onStatus({ phase: 'play-format' });
    return { ...(await withFooter(out.result)), nonce: String(nonce), sealSignature: out.signature };
  }
