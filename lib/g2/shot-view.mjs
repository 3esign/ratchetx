...
export function renderShotView(container, snapshot, { web3, summaryText, onSave, onRefresh } = {}) {
  ...
  const shot = snapshot.kind === 'account' ? snapshot.shot : archived ? snapshot.receipt.event.result : null;
  if (!shot) {
    const text = {
    ...
    const title = own(STATES, shot.state, 'state');
    ...
    for (const which of ['entry', 'exit']) {
      ...
      const available = !archived && allowedState.includes(shot.state) && nonzero(shot[which + 'TimepinResultHash']);
    ...
    root.append(prices);
    if (archived) root.append(node('p', 'g2-provenance', 'Prices, hit or miss, XP, execution details and raw Shot bytes are not included in this compact receipt.'));
    ...
    root.append(node('p', 'g2-provenance', archived
      ? (snapshot.checks?.logProvenance === true ? 'Read from successful transaction logs reported by this RPC. ' : 'Transaction log provenance has not been checked. ') + 'Result economics and the history commitment have not been independently checked.'
