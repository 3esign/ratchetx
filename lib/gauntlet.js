...
const GAUNTLET = Object.freeze({
  ...
  /* publicRuns injected dynamically */
  measurement: {
...
async function publicSpecAsync() {
  const ev = require('./evidence.js');
  const spec = JSON.parse(JSON.stringify(GAUNTLET));
  spec.publicRuns = await ev.getPublicRuns();
  spec.externalAudits = await ev.getExternalAudits();
  return spec;
}
...
function progressFromState(state, value) {
  ...
  return {
    ...
    terminal: open.length === 0 && !!latest,
    reasonCode: (open.length === 0 && latest) ? (latest.res === 'void' ? 'VOID' : (latest.res === 'hit' ? 'HIT' : 'MISS')) : null,
    expiry: open.length ? open[0].exp : (latest ? latest.exp : null),
    pollAfterSeconds: open.length ? Math.max(0, Math.ceil((open[0].exp - Date.now() + 65000) / 1000)) : 0,
    nextAction: next,
    receiptDigest: null,
...
module.exports = {
  publicSpecAsync, GAUNTLET, publicSpec, cleanHandle, progressFromState };
