...
- **Git over the bridge: move every lock, not just `index.lock`, and move it INSIDE `.git`.**
  Deletion is not granted, so `git` cannot clean up after itself. Before and after every git write:

      mkdir -p .git/ratchet-lock-quarantine
      for f in $(find .git -maxdepth 4 -name '*.lock'); do \
        mv "$f" ".git/ratchet-lock-quarantine/$(basename "$f").$.bak"; done

  Three details, each of which cost somebody an hour. `HEAD.lock`, `index.lock` and
  `objects/maintenance.lock` are all real and `index.lock` alone is not enough. The
  destination must be **under `.git`**: a quarantine directory at the repository root is a new
  top-level directory full of untracked files, and `scripts/check-release-safety.mjs` turns RED
  on exactly that - measured 2026-09-05, twenty failures, by me, with my own workaround. It is
  the ROOT_DIRS allowlist doing its job; do not widen it, move the files instead. And the `.bak`
  suffix matters: without it the quarantined files match `*.lock` and the next run re-quarantines
  its own quarantine.
- **Never pipe a `git` command into `head`.** The closed pipe kills git mid-write and leaves
  `index.lock` behind - which is the failure the recipe above exists to clean up. Measured the
  same day, same way.
